...
abstract: \singlespacing Porter and Wood (2022) assess the effects of factual corrections on a high-choice Facebook news feed environment using two innovative online survey experiments. Both experiments lead to large correction effects (improvements in accuracy.) In study 1, the correction effect was estimated to be .55  on a five point scale; in study 2 the correction effect was estimated to be .79 on a five point scale.  Both estimates were significant at the 0.001 level. We subject these results to a robustness replication where we 1) recode the dependent variables, 2) adjust the standard errors, 3) and  model the outcomes using a multilevel model. Under some of these procedures, the magnitude or the significance level of the correction effect estimates is slightly reduced. Still, we find the original estimates robust to a set of reasonable modeling choices.  
...
In this short paper, we subject these results to a robustness replication. We conducted three tests. Specifically, we first used an ordered probit instead of a linear regression to model the outcome. Next, to account for the nested structure of the data we adjust the standard errors by clustering at the individual level and the claim level. Finally, and for identical reasons, we model the outcome using a multilevel model, with varying intercepts for individuals, claims, and both. We find that in the first case,. In the second case, clustering standard errors at the individual level produce no change in the significance level. However, clustering at the claim level changes the significance level to 0.01. In the last case, we obtain slightly attenuated, though statistically significant, correction effects in the same direction (Study 1: 0.53 (p < .001), Study 2: 0.76 (p < .001)).  Overall, we find the original estimates robust to a set of quite reasonable, alternate modeling choices.  
...
We begin by plotting the distribution of the dependent across treatment conditions. [Reason] We note that the visually, the factual correction condition seems to push the distribution in the direciton of more accuracy for both conditions which is line with the findings. 
...
The original results are estimated from a linear regression model. However, the dependent variable, whether accuracy or truthfulness, is measured using an ordinal variable (1-5). To improve on this model, we assess whether running an ordered probit generates coefficients roughly in line with the magnitude and statistical significance of the original studies. 
...
What other stuff could be done?
